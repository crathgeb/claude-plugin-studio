...
export class CreateCommand {
  ...
  private async createMarketplace(): Promise<void> {
    ...
    const description = await this.prompt.askDescription('marketplace');
    const ownerName = await this.prompt.askOwnerName();
    const ownerEmail = await this.prompt.askOwnerEmail();

    await this.scaffolder.createMarketplace({
      name,
      description,
      ownerName,
  ...
  private async createPlugin(): Promise<void> {
    ...
    const description = await this.prompt.askDescription('plugin');

    await this.scaffolder.createPlugin({ name, description });
